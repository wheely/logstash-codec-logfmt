...
describe LogStash::Codecs::Logfmt do
  ...
  context '#decode' do
    it 'should decode valid logfmt data' do
      data = %(time=2016-12-27T16:15:00.108+00:00 level=error msg=MaxMindError response_status=401 response_body.code=AUTHORIZATION_INVALID response_body.error="Your user ID or license key could not be authenticated." progname=null logger=Class)
    ...
    context 'processing plain text' do
      let(:message) { "something containing level word that isn't json" }
      it 'falls back to plain text' do
        ...
        subject.decode(message) do |event|
          ...
          insist { event.get('_logfmtparsefailure') } == true
        end
        insist { decoded } == true
      end
